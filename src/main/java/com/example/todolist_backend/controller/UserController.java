...
import com.example.todolist_backend.exception.ExceptionResponse;
...
public class UserController {
    ...
    @PostMapping("/join")
    public ResponseEntity<?> join(@RequestBody UserJoinRequest joinRequestBody) { // <?> : 타입 와일드카드
       try{
           authService.join(joinRequestBody);
           return ResponseEntity.ok().body(ResponseDto.setSuccess("회원가입 성공했습니다.",null));
       }
       catch (IllegalStateException e){
           return new ResponseEntity<>(ResponseDto.setFailed(e.getMessage()), HttpStatus.CONFLICT);
       }
//       catch (UserAlreadyExistException e){
//            return ResponseEntity.status(HttpStatus.CONFLICT).body(ResponseDto.setFailed(e.getMessage()));
//       }
